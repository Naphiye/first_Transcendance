...
export async function ensureMrQiExists() {
    const existing = await db.select().from(users).where(eq(users.username, "MrQi"));
    if (existing.length === 0) {
        ...
        if (!newUser[0]) {
            throw new Error("Couldnt create MrQi user");
        }
        const adminID = await db.select({ id: users.id }).from(users).where(eq(users.username, "Pouet"));
        if (!adminID[0]) {
            console.error("Couldnt find Pouet user to add for MrQi's friends list");
            return;
        }
        // mrqi pending friendship with admin (Pouet)
        ...
        console.log("💜 MrQi created");
    } else {
        console.log("ℹ️ MrQi already exists");
    }
}